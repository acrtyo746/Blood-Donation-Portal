...
$stmt = $conn->prepare("SELECT name, email, phone, bloodgroup, gender, birthdate, weight, state, zipcode, district, area, landmark, password FROM registered_users WHERE email = ?");
if (!$stmt) {
    die("Error: " . $conn->error);
}
$stmt->bind_param("s", $email);
$stmt->execute();
$stmt->bind_result($name, $email, $phone, $bloodgroup, $gender, $birthdate, $weight, $state, $zipcode, $district, $area, $landmark, $password );
$stmt->fetch();
$stmt->close();
...
<head>
    ...
    <title>Blood Portal - Dashboard</title>
...
<footer class="footer">
    ...
    <div class="footer-bottom">
      <div class="container">
        <p class="copyright">
          &copy; 2024 All Rights Reserved by Blood_Donation_Drive_Portal
        </p>
        <ul class="social-list">
          <li>
            <a href="https://www.facebook.com/andro.pool.54?mibextid=ZbWKwL" class="social-link">
